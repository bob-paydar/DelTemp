# DelTemp
 ...
 DelTemp for Windows
 Program by: Bob.paydar@hp.com


Overview
--------
DelTemp is a lightweight console application that helps keep your 
Windows system clean by deleting unnecessary temporary files. 
...
TempCleaner is safe to use: files that are locked or in use will be skipped. 
It automatically removes read-only, hidden, and system attributes before 
deletion, ensuring files can be removed when possible.
...
-----------------------------------------------------------------------
 Notes
-----------------------------------------------------------------------
 - Files currently in use by Windows or applications cannot be deleted 
   and will be skipped automatically.
 - If elevation is cancelled when using --system, only the user’s %TEMP% 
...
-----------------------------------------------------------------------
 DelTemp v1.0
 Programmed by Bob.paydar@hp.com
